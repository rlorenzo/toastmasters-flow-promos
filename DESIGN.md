...
cannot be mistaken for an official TI property — the repo's non-affiliation notice depends
...
- **Ink Engrave** (#7C838A): stamped ordinals only, where low contrast is deliberate because the ordinal's information is duplicated at full contrast elsewhere (each station number reappears in its step head). Declared in `:root` but not yet applied — `.station .num` still stamps in ink-stamp.
- **Ink Stamp** (rgba(233,229,218,.3)): the stamped station ordinal as currently shipped — ivory at 30%, pressed into the plate rather than printed on it.
...
- **Label / Plate** (700, 0.685rem, tracking 0.17em, uppercase, `wdth` 84, `ink-faint`): silkscreened equipment labels — "Repository", "Read before posting". State words and badges are the same voice at 0.62–0.63rem / 0.15em tracking, coloured by state ink; table headers at 0.66rem.
...
- **Shell:** `width: min(100% - 2.5rem, 74rem)`, centered. One container for everything; no sidebar anywhere — the page's thesis explicitly refuses the docs-sidebar arrangement that would flatten the six steps into equals.
- **Measure:** prose is capped at 68ch (`--measure`), including inside full-width panels (`.step-body > p`, readout captions). Panels go wide; sentences do not.
...
- **Etched grids:** multi-cell surfaces (legend, clearance list) are `gap: 1px` grids over a `hairline` background inside a `hairline-strong` border — dividers are the frame showing through, not drawn borders. Legend columns: `repeat(auto-fit, minmax(15rem, 1fr))`.
...
- **Bolted plate** (`inset 0 1px 0 rgba(255,255,255,.075), inset 0 -8px 18px rgba(0,0,0,.28)`): rail stations — no drop shadow, because a plate is fastened to the frame, not floating above it.
...
`currentColor` on hover — no colour change, no transition. Interactive focus is always
`outline: 2px solid` `ink` with 2px offset. There are no buttons and no form inputs on
the page; do not invent styles for them from this document.

## Do's and Don'ts

### Do:

- **Do** assign every step, station, and badge exactly one state class (`go` / `hold` / `spend`) and let the class colour lamp, word, and badge together — never colour those pieces individually.
...
- **Do** hold prose to the 68ch measure even inside wide panels, and keep all page-level spacing fluid (`clamp()`), reserving exact pixels for hardware details (fasteners, rail gaps, hairlines).
